...
//  private int validityAllocationSizeInBytes;
  ...
  public List<ArrowBuf> getFieldBuffers() {
    ...
    add(result);//result.add(validityBuffer);
  ...
  private void setReaderAndWriterIndex() {
    setReaderIndex(0); //validityBuffer.readerIndex(0);
    offsetBuffer.readerIndex(0);
    if (valueCount == 0) {
      setWriterIndex(0);//validityBuffer.writerIndex(0);
...
//  }
  ...
  private class TransferImpl implements TransferPair {
    ...
    public void transfer() {
      ...
      to.setValidityBuffer(transferOwnership(to.allocator).buffer);
  ...
  private int getValidityAndOffsetValueCapacity() {
    ...
    final int validityBufferValueCapacity  = (int) (getValidityBufferCapacity() * 8L);
    return Math.min(offsetValueCapacity, validityBufferValueCapacity);
  }
